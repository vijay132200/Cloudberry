...
router.get("/patients/:id/dashboard", async (req, res) => {
  try {
    ...
    const last7PostGlu = postGlucoseSeries.slice(-7).map(g => g.value);
    const avgPostGlucose = last7PostGlu.length > 0 ? Math.round(last7PostGlu.reduce((a, b) => a + b, 0) / last7PostGlu.length) : null;
    ...
    if (hasEnoughData) {
      insights = [];
      const lowE = last7.filter((c: typeof allCheckins[number]) => energyMap[c.energyLevel?.toLowerCase()] === 1).length;
      const skipped = last7.filter((c: typeof allCheckins[number]) => !c.activityCompleted).length;
    ...
    res.json({
      ...
      streak, weightChange, avgGlucose, avgPostGlucose, timeInRange,
